...
# Build the application in release mode (optimized, less memory)
...
# Create directories for JVT application
RUN mkdir -p /var/lib/pmtiles \
    && mkdir -p /var/log/tiles

# Create non-root user for running the application
RUN useradd -m -u 1001 -s /bin/bash tiles || echo "User already exists"
RUN chown -R tiles:tiles /var/lib/pmtiles /var/log/tiles
