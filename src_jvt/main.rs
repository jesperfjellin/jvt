...
async fn main() -> Result<()> {
    ...
    info!("Starting JVT core worker (Incremental Vector Tiles)");
    ...
    // Wait for worker to complete (or fail)
...
async fn run_worker_loop(
    ...
    info!("Starting worker loop (process on NOTIFY; periodic fallback)");
    ...
    loop {
        ...
        match listener.wait_for_notification(Duration::from_secs(5)).await {
            Ok(Some(notification)) => {
                info!(
                    "Received change notification on '{}' ({} bytes payload) - processing",
                ...
                match process_pending_tiles_batch(processor, mvt_generator, pmtiles_writer).await {
                    Ok(processed_count) => {
                        if processed_count > 0 {
                            info!("Processed {} tiles triggered by notification", processed_count);
                        }
                    }
                    Err(e) => {
            ...
            Ok(None) => {
                ...
                match process_pending_tiles_batch(processor, mvt_generator, pmtiles_writer).await {
                    Ok(processed_count) => {
                        if processed_count > 0 {
                            tracing::info!("Processed {} tiles on periodic check", processed_count);
                        } else {
...
async fn process_pending_tiles_batch(
    ...
    loop {
        ...
        batch_count += 1;
        info!("Processing {} tiles (chunk #{})...", batch.len(), batch_count);
    ...
    if total_processed > 0 {
        info!("Processed {} total tiles this cycle", total_processed);
    }
