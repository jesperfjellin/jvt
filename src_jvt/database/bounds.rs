...
impl BoundsDetector {
    ...
    pub async fn detect_data_bounds(&self) -> Result<GeographicBounds> {
        tracing::info!(
            "Detecting data bounds for tables: {:?}",
            self.config.geometry.tables
        );

        // Build query to get extent from each table separately, then combine
        let mut extent_parts = Vec::new();

        if self.config.geometry.tables.is_empty() {
        ...
        if min_lon.is_none() || min_lat.is_none() || max_lon.is_none() || max_lat.is_none() {
            anyhow::bail!("No geometry data found in configured tables; cannot determine bounds");
        }
    ...
    pub fn generate_tile_coverage(&self, bounds: &GeographicBounds, zoom: u8) -> Vec<TileCoord> {
        ...
        let min_y = ((1.0
            - (bounds.max_lat.to_radians().tan() + 1.0 / bounds.max_lat.to_radians().cos()).ln()
                / std::f64::consts::PI)
            / 2.0
            * n as f64)
            .floor() as u32;
        let max_y = ((1.0
            - (bounds.min_lat.to_radians().tan() + 1.0 / bounds.min_lat.to_radians().cos()).ln()
                / std::f64::consts::PI)
            / 2.0
            * n as f64)
            .floor() as u32;
