...
async fn main() -> Result<()> {
    ...
    info!(
        "Configuration loaded: z{}-z{}",
        config.tiles.min_zoom, config.tiles.max_zoom
    );
    ...
    if !all_tile_data.is_empty() {
        info!(
            "Writing {} tiles to PMTiles archive...",
            all_tile_data.len()
        );
        pmtiles_writer.write_tiles(&all_tile_data).await?;
        info!(
            "Successfully wrote {} tiles to PMTiles archive",
            all_tile_data.len()
        );
    } else {
